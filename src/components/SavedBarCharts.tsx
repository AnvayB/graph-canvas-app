...
import { Eye, Trash, BarChart3, Undo2 } from 'lucide-react';
...
const SavedBarCharts = () => {
  ...
  const loadSavedCharts = async () => {
    try {
      setLoading(true);
      const savedCharts = await loadCharts('bar');
      setCharts(savedCharts);
    } catch (error) {
      toast({
        title: "Error loading charts",
        description: "Failed to load saved bar charts",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleDeleteChart = async (id: string) => {
    ...
    try {
      ...
      toast({
        title: "Chart deleted",
        description: "Bar chart has been deleted successfully",
        action: (
  ...
  const handleUndoDelete = async (chartData: ChartData) => {
    try {
      ...
      toast({
        ...
        description: "Bar chart has been restored successfully",
      });
    } catch (error) {
      toast({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Saved Bar Charts</h1>
          <p className="text-gray-600">Manage your saved bar chart configurations</p>
        </div>
        ...
        {charts.length === 0 ? (
          <Card>
            <CardContent className="text-center py-12">
              ...
              <p className="text-gray-600 mb-4">You haven't saved any bar charts yet.</p>
              <Button onClick={() => navigate('/bar-chart')}>
